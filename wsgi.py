...
class Main:
    ...
    def main():
        ...
        try:
            routes_Query.run(app)
        except BaseException as _g:
            e = haxe_Exception.caught(_g)
            tmp = e.get_stack()
            print(str(("null" if ((tmp is None)) else haxe__CallStack_CallStack_Impl_.toString(tmp))))
        try:
            routes_PPMViewer.run(app)
        except BaseException as _g:
            e = haxe_Exception.caught(_g)
            tmp = e.get_stack()
            print(str(("null" if ((tmp is None)) else haxe__CallStack_CallStack_Impl_.toString(tmp))))
        try:
            routes_Img2Digi.run(app)
        except BaseException as _g:
            e = haxe_Exception.caught(_g)
            tmp = e.get_stack()
            print(str(("null" if ((tmp is None)) else haxe__CallStack_CallStack_Impl_.toString(tmp))))
...
class routes_Home:
    ...
    def run(app):
        ...
        home_route = _hx_local_0
        app.add_url_rule("/home","home_route",home_route)


class routes_Img2Digi:
    _hx_class_name = "routes.Img2Digi"
    __slots__ = ()
    _hx_statics = ["run"]
    ...
    def run(app):
        ...
        digi_route = _hx_local_0
        app.add_url_rule("/digscreen","digi_route",digi_route)


class routes_PPMViewer:
    _hx_class_name = "routes.PPMViewer"
    __slots__ = ()
    _hx_statics = ["run"]
    ...
    def run(app):
        ...
        ppmviewer_route = _hx_local_0
        app.add_url_rule("/ppm","ppmviewer_route",ppmviewer_route)


class routes_Query:
    _hx_class_name = "routes.Query"
    __slots__ = ()
    _hx_statics = ["run"]
    ...
    def run(app):
        def _hx_local_0():
            webhook_url = libs_Request.args.get("webhook",None)
            content = libs_Request.args.get("content",None)
            if ((webhook_url is None) or ((content is None))):
                return libs_Api.punt("You need to provide the 'webhook' and 'content' keys!")
            bot_name = libs_Request.args.get("name",None)
            avatar = libs_Request.args.get("avatar",None)
            dict_to_send = dict()
